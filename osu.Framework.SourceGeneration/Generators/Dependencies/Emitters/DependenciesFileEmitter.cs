            ...
            foreach (ConstructorDeclarationSyntax origConstructor in Target.ClassSyntax.Members.OfType<ConstructorDeclarationSyntax>())
            {
                ...
                yield return
                    origConstructor
                        .WithParameterList(
                            SyntaxFactory.ParameterList(
                                SyntaxFactory.SeparatedList(origRequiredParams)))
                        .AddParameterListParameters(
                            SyntaxFactory.Parameter(
                                             SyntaxFactory.Identifier("dependencies"))
                                         .WithType(
                                             SyntaxFactory.GenericName(
                                                              SyntaxFactory.Identifier("global::osu.Framework.Allocation.DependencyList"))
                                                          .WithTypeArgumentList(
                                                              SyntaxFactory.TypeArgumentList(
                                                                  SyntaxFactory.SeparatedList(
                                                                      depsRequiredParams.Select(p => p.Type!))))))
                        .AddParameterListParameters(
                            origOptionalParams.ToArray())
                        .WithInitializer(
